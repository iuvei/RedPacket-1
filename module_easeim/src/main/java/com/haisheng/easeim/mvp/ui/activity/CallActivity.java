...
public abstract class CallActivity <P extends IPresenter> extends BaseSupportActivity<P> {
    ...
    protected UserInfo mUserEntity;
...
//                    CallRecordDao.getInstance().saveCallRecordEntity();
    ...
    protected void saveCallRecord() {
        ...
        recordEntity.setUsername(mUserEntity.getHxId());
        recordEntity.setNickname(mUserEntity.getNickname());
        recordEntity.setAvatarUrl(mUserEntity.getAvatarUrl());
        recordEntity.setType(callType == 0 ? CallRecordEntity.CallTypeStatus.VOICE.ordinal() : CallRecordEntity.CallTypeStatus.VIDEO.ordinal());
        recordEntity.setMessage(txtBody.getMessage());
        recordEntity.setTimestamp(System.currentTimeMillis());

        CallRecordDao.getInstance().saveCallRecordEntity(recordEntity);
    }
